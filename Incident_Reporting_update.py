...
def fetch_weather_data(location):
    ...
    try:
        weather_response = requests.get(f"https://api.weatherapi.com/v1/current.json?key=your_weather_api_key&q={location}")
        if weather_response.status_code == 200:
            weather_data = weather_response.json()
            return f"{weather_data['current']['condition']['text']} at {weather_data['current']['temp_c']}°C"
