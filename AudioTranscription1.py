...
uploaded_audio_file = st.file_uploader("Upload an audio file", type=["mp3", "wav", "m4a"])
...
def abstract_summary_extraction(transcription):
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Summarize this audio file into a text report. Be sure to include all details that are in the recording the user uploaded."},
...
if uploaded_audio_file:
    ...
    if option == "Summary":
        st.write("**Summary:**", abstract_summary)
